...
export function Footer({selected,setSelected,data}){
      ...
      const countValues=data.reduce((acc,todo)=>{
    ...
    <TouchableOpacity onPress={()=>setSelected(constants.all)}>
        <Text style={selected_text_style(constants.all)}>All {data.length}</Text>    
    </TouchableOpacity>
    <TouchableOpacity onPress={()=>setSelected(constants.in_progress)}>
        <Text style={selected_text_style(constants.in_progress)}>In Progress {countValues.inProgress}</Text>    
    </TouchableOpacity>
    <TouchableOpacity onPress={()=>setSelected(constants.done)}>
        <Text style={selected_text_style(constants.done)}>Done {countValues.completed}</Text>    
    </TouchableOpacity>
    </View>
    );
}